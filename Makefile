...
go:
	...
	@docker run --rm -v "${PWD}":/apm-proto -w /apm-proto/go/collectorpb golang:1.24.0 sh -c "go mod init github.com/solarwinds/apm-proto/go/collectorpb && go mod tidy"
	@docker run --user `id -u` --rm -v "${PWD}/go/collectorpb":/src -w /src vektra/mockery:v2.53.5 --all --case=underscore
